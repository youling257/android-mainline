...
static const struct dmi_system_id byt_rt5640_quirk_table[] = {
	{	/* Acer Iconia Tab 8 W1-810 */
		.matches = {
			...
			DMI_MATCH(DMI_PRODUCT_NAME, "Iconia W1-810"),
		},
	...
	{	/* Lamina I8270 / T701BR.SE */
		.matches = {
			DMI_MATCH(DMI_BOARD_VENDOR, "Lamina"),
			DMI_MATCH(DMI_BOARD_NAME, "T701BR.SE"),
		},
		.driver_data = (void *)(BYTCR_INPUT_DEFAULTS |
					BYT_RT5640_MONO_SPEAKER |
					BYT_RT5640_JD_NOT_INV |
...
static int byt_rt5640_init(struct snd_soc_pcm_runtime *runtime)
{
	...
	if (BYT_RT5640_JDSRC(byt_rt5640_quirk)) {
		...
		snd_jack_set_key(priv->jack.jack, SND_JACK_BTN_0,
				 KEY_PLAYPAUSE);
		snd_soc_codec_set_jack(codec, &priv->jack, NULL);
	}
...
static int byt_rt5640_suspend(struct snd_soc_card *card)
{
	...
	list_for_each_entry(component, &card->component_dev_list, card_list) {
		if (!strcmp(component->name, byt_rt5640_codec_name)) {
			...
			dev_info(codec->dev, "disabling jack detect before suspend\n");
			snd_soc_codec_set_jack(codec, NULL, NULL);
			break;
		}
	}
...
static int byt_rt5640_resume(struct snd_soc_card *card)
{
	...
	list_for_each_entry(component, &card->component_dev_list, card_list) {
		if (!strcmp(component->name, byt_rt5640_codec_name)) {
			struct snd_soc_codec *codec = snd_soc_component_to_codec(component);
			dev_info(codec->dev, "re-enabling jack detect after resume\n");
			snd_soc_codec_set_jack(codec, &priv->jack, NULL);
			break;
		}
	}
