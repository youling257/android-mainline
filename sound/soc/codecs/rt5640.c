...
static void rt5640_set_ovcd_params(struct snd_soc_component *component)
{
	struct rt5640_priv *rt5640 = snd_soc_component_get_drvdata(component);
			   ...
			   struct snd_soc_jack *jack, void *data)
{
	...
	ret = devm_add_action_or_reset(component->dev, rt5640_cancel_work, rt5640);
	if (ret)
		return ret;

	rt5640->jack = jack;

	ret = devm_request_irq(component->dev, rt5640->irq, rt5640_irq,
...
static int rt5640_suspend(struct snd_soc_component *component)
{
	...
	if (rt5640->jack) {
		dev_info(component->dev, "suspend, disabling irq\n");
		disable_irq(rt5640->irq);
		rt5640_cancel_work(rt5640);

		if (rt5640->jack->status & SND_JACK_MICROPHONE) {
			dev_info(component->dev, "suspend, disabling button monitoring\n");
			snd_soc_jack_report(rt5640->jack, 0, SND_JACK_BTN_0);
			rt5640_disable_micbias1_ovcd_irq(component);
			rt5640_disable_micbias1_for_ovcd(component);
		}
...
static int rt5640_resume(struct snd_soc_component *component)
{
	...
	if (rt5640->jack) {
		if (rt5640->jack->status & SND_JACK_MICROPHONE) {
			dev_info(component->dev, "resume, restarting button monitoring\n");
			rt5640_enable_micbias1_for_ovcd(component);
			rt5640_enable_micbias1_ovcd_irq(component);
		}
		dev_info(component->dev, "resume, re-enabling irq\n");
