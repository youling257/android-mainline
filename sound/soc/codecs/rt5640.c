...
static bool rt5640_micbias1_ovcd(struct snd_soc_codec *codec)
{
	...
	dev_info(codec->dev, "irq ctrl2 %#04x\n", val);
...
static int rt5640_detect_headset(struct snd_soc_codec *codec)
{
	...
	for (i = 0; i < JACK_DETECT_MAXCOUNT; i++) {
		...
		if (rt5640_micbias1_ovcd(codec)) {
			 ...
			 */
			dev_info(codec->dev, "jack mic-gnd shorted\n");
			headset_count = 0;
			headphone_count++;
			if (headphone_count == JACK_DETECT_COUNT)
				return SND_JACK_HEADPHONE;
		} else {
			dev_info(codec->dev, "jack mic-gnd open\n");
			headphone_count = 0;
			headset_count++;
...
static int rt5640_set_jack(struct snd_soc_codec *codec,
...
static int rt5640_probe(struct snd_soc_codec *codec)
{
	 ...
	 */
	if (device_property_read_bool(codec->dev, "realtek,in1-differential"))
	...
	if (device_property_read_u32(codec->dev,
		...
		if (val <= RT5640_JD_SRC_GPIO4)
			...
			dev_warn(codec->dev, "Warning: Invalid jack-detect-source value: %d, leaving jack-detect disabled\n",
				 val);
	}

	if (!device_property_read_bool(codec->dev, "realtek,jack-detect-not-inverted"))
