...
static void rt5651_enable_micbias1_for_ovcd(struct snd_soc_codec *codec)
{
...
static void rt5651_disable_micbias1_for_ovcd(struct snd_soc_codec *codec)
{
	struct snd_soc_dapm_context *dapm = snd_soc_codec_get_dapm(codec);
...
static int rt5651_detect_headset(struct snd_soc_codec *codec)
{
	...
	for (i = 0; i < JACK_DETECT_MAXCOUNT; i++) {
		...
		if (rt5651_micbias1_ovcd(codec)) {
			 ...
			 */
			dev_dbg(codec->dev, "mic-gnd shorted\n");
			headset_count = 0;
			headphone_count++;
			if (headphone_count == JACK_DETECT_COUNT)
				return SND_JACK_HEADPHONE;
		} else {
			dev_dbg(codec->dev, "mic-gnd open\n");
			headphone_count = 0;
			headset_count++;
...
static void rt5651_jack_detect_work(struct work_struct *work)
{
	...
	if (rt5651_jack_inserted(rt5651->codec)) {
		rt5651_enable_micbias1_for_ovcd(rt5651->codec);
		report = rt5651_detect_headset(rt5651->codec);
		rt5651_disable_micbias1_for_ovcd(rt5651->codec);
	}
			   ...
			   struct snd_soc_jack *hp_jack, void *data)
{
	...
	switch (rt5651->jd_src) {
	case RT5651_JD1_1:
		snd_soc_update_bits(codec, RT5651_JD_CTRL2,
	...
	case RT5651_JD_NULL:
		return 0;
	default:
		dev_err(codec->dev, "Currently only JD1_1 / JD1_2 / JD2 are supported\n");
		return -EINVAL;
	}
	...
	snd_soc_write(codec, RT5651_PR_BASE + RT5651_BIAS_CUR4,
 ...
 * property parsing MUST be done from the codec driver's probe function,
 ...
 */
static void rt5651_apply_properties(struct snd_soc_codec *codec)
{
	struct rt5651_priv *rt5651 = snd_soc_codec_get_drvdata(codec);
	u32 val;
	...
	if (device_property_read_u32(codec->dev,
		...
		case 2000:
			...
			break;
		default:
			dev_warn(codec->dev, "Warning: Invalid over-current-threshold-microamp value: %d, defaulting to 2000uA\n",
				 val);
		}
	}
	...
	if (device_property_read_u32(codec->dev,
		...
		if (val <= RT5651_OVCD_SF_1P5)
			...
			dev_warn(codec->dev, "Warning: Invalid over-current-scale-factor value: %d, defaulting to 0.75\n",
				 val);
	}
}
