...
int do_truncate2(struct vfsmount *mnt, struct user_namespace *mnt_userns, struct dentry *dentry,
...
long do_sys_ftruncate(unsigned int fd, loff_t length, int small)
{
	...
	if (!error)
		error = do_truncate2(mnt, file_mnt_user_ns(f.file), dentry, length,
...
SYSCALL_DEFINE1(fchdir, unsigned int, fd)
{
	...
	error = file_permission2(f.file->f_path.mnt, f.file, MAY_EXEC | MAY_CHDIR);
	if (!error)
...
retry_deleg:
	...
	error = notify_change2(path->mnt, mnt_user_ns(path->mnt), path->dentry,
...
retry_deleg:
	...
	if (!error)
		error = notify_change2(path->mnt, mnt_userns, path->dentry, &newattrs,
				      &delegated_inode);
	inode_unlock(inode);
	if (delegated_inode) {
		error = break_deleg_wait(&delegated_inode);
